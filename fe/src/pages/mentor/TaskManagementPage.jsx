...
import CreateSprintModal from "../taskManagement/CreateSprintModal";
import EditSprintModal from "../taskManagement/EditSprintModal";
import SprintReviewModal from "../taskManagement/SprintReviewModal";
...
function TaskManagementPage() {
  ...
  // --- Modal Handlers (Task) ---
  const handleOpenCreateTaskModal = async (status) => {
    // Made async
    ...
    if (selectedSprintId) {
      ...
      if (sprint && sprint.teamId) {
        try {
          ...
          console.error(
            "Không thể lấy thành viên nhóm để tạo nhiệm vụ:",
            error
          );
      ...
      // If no sprint selected, but team is, try to get members for the team
      try {
        const response = await TeamApi.getTeamById(selectedTeamId);
        setAssignableMembers(response.data.members || []);
      } catch (error) {
        console.error(
          "Không thể lấy thành viên nhóm để tạo tác vụ (không có sprint):",
          error
        );
        setAssignableMembers([]);
      }
  ...
  const handleOpenDetailModal = async (task) => {
    ...
    if (task.sprint_Id) {
      ...
      // If task is in backlog, use the currently selected team
      try {
        const response = await TeamApi.getTeamById(selectedTeamId);
        setAssignableMembers(response.data.members || []);
      } catch (error) {
        console.error(
          "Không thể lấy thành viên nhóm cho nhiệm vụ tồn đọng:",
          error
        );
        setAssignableMembers([]);
      }
  ...
  const columnsForBoard = {
    BACKLOG: {
      name: "Backlog",
      items: backlogTasks.filter((t) => t.status !== "CANCELLED"),
    },
    TODO: {
      name: "To Do",
